...
window.__processVideo = function(videoElement) {
        ...
        html = '<object type="application/x-shockwave-flash" width="100%" height="100%">'
                ...
                + ' <param name="movie" value="lib/flowplayer-3.1.1.swf" />'
                + '</object>';
        div.innerHTML = html;
...
if ("behavior" in videoElement.style) {
	...
	style.type = 'text/css';
	style.styleSheet.cssText = "video { behavior: url(lib/video4all.htc); }";
...
if ("MozBinding" in videoElement.style) {
        var style = document.createElement('style');
        style.type = 'text/css';
        style.textContent = 'video { display: block; -moz-binding: url("lib/video4all.xml#video4all"); }';
